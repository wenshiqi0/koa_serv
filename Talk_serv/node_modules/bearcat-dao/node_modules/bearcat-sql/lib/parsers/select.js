...
function divideTokens(tokens) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (lev !== 0 || type !== Lexter.types.KEYWORD) {
        continue;
      }
...
function parseWhere(tokens) {
  ...
  var relate = parts[0][1];
  if (parts[0].length == 3 && relate.text.toLowerCase() === "between") {
    parts[0].push(parts[1][0]);
    parts.pop();
  }
...
function parseOrderby(tokens) {
  ...
  for (var i = 0; i < parts.length; i++) {
    var tmp = parts[i];
    var type;
    var last = tmp[tmp.length - 1].text;
...
exports.createObj = function(sql) {
    ...
    for (var i = 0; i < tokenGroups.length; i++) {
      ...
      if (parts["column"]) {
        res.column = parseColumn(parts["column"]);
      }
      if (parts["source"]) {
        res.source = parseSource(parts["source"]);
      }
      if (parts["joinmap"]) {
        res.joinmap = parseJoinmap(parts["joinmap"]);
      }
      if (parts["where"]) {
        res.where = parseWhere(parts["where"]);
      }
      if (parts["groupby"]) {
        res.groupby = parseGroupby(parts["groupby"]);
      }
      if (parts["orderby"]) {
        res.orderby = parseOrderby(parts["orderby"]);
      }
      if (parts["limit"]) {
        res.limit = parseLimit(parts["limit"]);
      }
      result.push(res);
    }
