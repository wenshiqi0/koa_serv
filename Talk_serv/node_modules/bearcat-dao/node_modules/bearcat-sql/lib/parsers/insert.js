...
function divideTokens(tokens) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (lev !== 0 || tokens[i].type !== Lexter.types.KEYWORD) {
        continue;
      }
...
function parseSource(tokens) {
  ...
  if (!(/^insert$/i.test(tokens.shift().text))) {
    throw new Error("no keyword 'insert'");
  }
  if (!(/^into$/i.test(tokens.shift().text))) {
    throw new Error("no keyword 'into'");
  }
...
exports.createObj = function(sql) {
    ...
    if (parts["values"]) {
      res.values = parseValues(parts["values"]);
    }
