...
var RELATE_MAP = {};
for (var key in RELATE) {
  var value = RELATE[key];
  RELATE_MAP[value] = key;
}
exports.RELATE_MAP = RELATE_MAP;
...
function parseOneSource(part) {
  ...
  if (idx === -1) {
    ...
    if (part[1].text !== "(") {
      throw new Error("something wrong in 'source' part");
    }
    var innerLev = 0;
    var j;
    ...
    if (j === part.length) {
      throw new Error("lack parentheses in 'source' part");
    }
    //列入db.table这样的源
  } else {
  ...
  if (part.length === 0) {
    ...
    if (part.length >= 3 || (part.length === 2 && !(/^as$/i.test(part[0].text))) || (part[part.length - 1].type !== Lexter.types.KEYWORD && part[part.length - 1].type !== Lexter.types.VARIABLE)) {
      throw new Error("something wrong in 'source' part");
    }
    res = {
